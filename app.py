...
def render_metrics(stock_data: Dict):
    """Render KPI metrics for selected stock"""
    col1, col2, col3, col4 = st.columns(4)
...
def render_results_table(df_results: pd.DataFrame):
    ...
    display_columns = ['ticker', 'close', 'score', 'signal', 'rsi', 'sentiment']
    display_df_filtered = display_df[display_columns]
    
    # Rename columns for better display
    display_df_filtered.columns = ['Ticker', 'Price', 'Score', 'Signal', 'RSI', 'Sentiment']
    ...
    if selected_history_ticker:
        ...
        if not df_history.empty:
            ...
            with col1:
                ...
                st.dataframe(
                    df_history[['scan_time', 'close', 'score', 'signal']].head(10),
                    hide_index=True,
                    use_container_width=True
                )
...
def main():
    ...
    st.markdown("*Real-time stock screening with technical analysis*")
    st.markdown("---")
    ...
    if not df_results.empty:
        ...
        if st.session_state.selected_ticker:
            ...
            with col2:
                # Show chart for ANY selected stock (not just BUY signals)
                st.subheader(f"📊 Chart: {st.session_state.selected_ticker}")
    ...
    st.markdown("""
    <div style='text-align: center; color: gray; padding: 20px;'>
        <p>📊 Stock Trading Dashboard v1.0 | Built with Streamlit</p>
        <p style='font-size: 12px;'>Data provided by Yahoo Finance | Not financial advice</p>
    </div>
    """, unsafe_allow_html=True)
