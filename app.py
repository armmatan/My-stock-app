...
def render_sidebar_controls() -> Dict:
    ...
    st.sidebar.info(f"💡 Max risk per trade: ${account_size * (risk_percent/100):,.2f}")
    
    # Perplexity AI Section
    st.sidebar.markdown("---")
    st.sidebar.subheader("🤖 AI Insights")
    
    with st.sidebar.expander("ℹ️ About Perplexity AI"):
        st.markdown("""
    ...
    if perplexity_key:
        ...
        st.sidebar.info("💡 Add key for AI analysis")
    
    # Alpha Vantage Section
    st.sidebar.markdown("---")
    st.sidebar.subheader("🔬 Fundamentals")
    
    with st.sidebar.expander("ℹ️ About Fundamental Data"):
        st.markdown("""
        **Expert Mode** adds:
        - P/E & PEG Ratios
        - Revenue Growth
    ...
    if api_key:
        st.sidebar.success("✅ Premium Fundamentals")
    else:
...
def render_stock_details(stock_data: Dict):
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["📈 Technical", "💼 Fundamentals", "⚠️ Risk Management", "🤖 AI Insights"])
    ...
    with tab3:
        ...
        if 'atr_stop_loss' in stock_data:
            ...
            <div style="
                ...
                margin: 10px 0;
            ">
                <h4 style="margin-top: 0; color: #856404;">🛡️ ATR-Based Stop Loss</h4>
                ...
                <p style="font-size: 12px; color: #666; margin-top: 10px;">
                    Calculated as: Price - (1.5 × ATR) = ${current_price:.2f} - (1.5 × ${atr:.2f})
                </p>
            </div>
            """, unsafe_allow_html=True)
        ...
        if 'position_shares' in stock_data:
            ...
            <div style="
                ...
                margin: 10px 0;
            ">
                <h4 style="margin-top: 0; color: #0c5460;">💰 Recommended Position Size</h4>
        ...
        if recommendation in ['Strong Buy', 'Buy']:
            ...
            st.info("💡 Enter position with calculated share size and set stop loss immediately")
    ...
    with tab4:
        ...
        if 'ai_sentiment' in stock_data and stock_data['ai_sentiment']:
            ...
            <div style="
                ...
                margin: 10px 0;
            ">
                <h4 style="margin-top: 0; color: #1565C0;">🤖 Perplexity AI Analysis</h4>
            ...
            st.info("💡 Enable Perplexity AI in sidebar to get AI-powered sentiment analysis")
            st.markdown("""
            **What you'll get:**
        ...
        # Market context
        st.markdown("---")
        st.markdown("**📰 Manual Research Checklist:**")
...
def render_results_table(df_results: pd.DataFrame, gsheets_conn: Optional[GSheetsConnection] = None):
    ...
    # Check if 'recommendation' column exists (for backward compatibility)
    has_recommendation = 'recommendation' in df_results.columns
    ...
    st.subheader("📊 Pro Analysis Results")
    ...
    with col1:
        if has_recommendation:
            ...
            strong_buy = len(df_results[df_results['signal'] == 'BUY'])
    ...
    with col3:
        if has_recommendation:
            ...
            neutral_count = len(df_results[df_results['signal'] == 'HOLD'])
    ...
    for _, row in df_results.iterrows():
        ...
        if has_recommendation:
            display_row['Expert Rec'] = row['recommendation']
        
        # Add other fields
        display_row['RSI'] = f"{row['rsi']:.1f}"
        
        if 'atr' in row and pd.notna(row['atr']):
            display_row['ATR'] = f"{row['atr']:.2f}"
        
        if 'position_shares' in row and pd.notna(row['position_shares']):
            display_row['Position'] = f"{int(row['position_shares'])} shares"
        
        if 'risk_reward_ratio' in row and pd.notna(row['risk_reward_ratio']) and row['risk_reward_ratio'] > 0:
            display_row['R/R'] = f"{row['risk_reward_ratio']:.2f}"
        else:
            display_row['R/R'] = "N/A"
    ...
    if has_recommendation:
        column_config["Expert Rec"] = st.column_config.TextColumn("Expert Rec", width="medium")
    
    if 'ATR' in final_df.columns:
        column_config["ATR"] = st.column_config.TextColumn("ATR", width="small", help="Average True Range")
    
    if 'Position' in final_df.columns:
        column_config["Position"] = st.column_config.TextColumn("Position", width="medium", help="Recommended shares")
    
    if 'R/R' in final_df.columns:
        column_config["R/R"] = st.column_config.TextColumn("R/R", width="small", help="Risk/Reward Ratio")
    ...
    if has_recommendation:
        ...
        **Legend:**  
        ...
        📈 **Sparkline** shows 7-day score trend (green=up, red=down)
        """)
    else:
        st.markdown("""
        **Legend:**  
        🟢 **BUY** (Score 70+) | 🟡 **HOLD** (Score 30-70) | 🔴 **SELL** (Score <30)
        
        💡 Run a new scan to see Pro Edition features (ATR Stop Loss, Position Sizing, AI Insights)
        """)
...
def main():
    ...
    ">
        ...
        <p style="color: #e0e0e0; text-align: center; margin: 10px 0 0 0;">
            Pro Edition • ATR Stop Loss • Position Sizing • AI Insights • Trend Tracking
        </p>
    </div>
    """, unsafe_allow_html=True)
    ...
    if controls['run_scan_clicked']:
        if not controls['selected_tickers']:
            ...
            if controls['perplexity_key']:
                status_items.append("🤖 AI Insights")
            if controls['api_key']:
                status_items.append("🔬 Premium Fundamentals")
            status_items.append(f"💰 Risk: {controls['risk_percent']}%")
            status_items.append("📊 ATR Stop Loss")
            
            st.info(f"**Pro Mode Active:** {' | '.join(status_items)}")
            
            with st.spinner("🔍 Running professional stock screener..."):
                ...
                if results:
                    ...
                    st.success(f"✅ Pro scan completed! Analyzed {len(results)} stocks with ATR risk management")
                else:
    ...
    <div style='text-align: center; color: gray; padding: 20px;'>
        <p><strong>📊 Stock Trading Dashboard v4.0 - Pro Edition</strong></p>
        <p style='font-size: 12px;'>
            ATR Stop Loss • Position Sizing • AI Insights • Trend Tracking
        </p>
        <p style='font-size: 12px;'>
            Built with Streamlit • Data: Yahoo Finance + Alpha Vantage + Perplexity AI
        </p>
